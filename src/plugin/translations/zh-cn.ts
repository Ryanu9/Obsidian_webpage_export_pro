...
export const language: i18n =
{
	...
	settings:
	{
		...
		backgroundImage: {
			...
			lightPathLabel: "亮色主题 - 本地图片路径",
			lightPathDescription: "亮色主题背景图片的本地路径（支持GIF动图）",
			lightUrlLabel: "亮色主题 - 图片URL",
			lightUrlDescription: "亮色主题背景图片的URL地址（同时设置时优先使用URL）",
			darkPathLabel: "暗色主题 - 本地图片路径",
			darkPathDescription: "暗色主题背景图片的本地路径（支持GIF动图）",
			darkUrlLabel: "暗色主题 - 图片URL",
			darkUrlDescription: "暗色主题背景图片的URL地址（同时设置时优先使用URL）",
			blurLabel: "背景模糊度",
			blurDescription: "模糊程度（像素），0 = 不模糊",
			opacityLabel: "背景透明度",
			opacityDescription: "背景图片透明度（0 = 完全透明，1 = 完全不透明）",
		},
		siteName: {
			title: "站点名称",
