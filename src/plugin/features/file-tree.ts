...
export class FileTree extends Tree {
	...
	protected async populateTree() {
		...
		this.updateFolderCountsRecursive(this);
	}
	...
	/**
	 * 递归更新文件夹及其所有子文件夹的文件计数
	 * 返回该文件夹及其所有子文件夹中的总文件数
	 */
	private updateFolderCountsRecursive(parent: FileTree | FileTreeItem): number {
		let totalFiles = 0;

		for (const child of parent.children) {
			if (child.isFolder) {
				// 首先递归获取子文件夹的计数
				const childFileCount = this.updateFolderCountsRecursive(child);
				child.fileCount = childFileCount; // 存储到该项目对象中
				totalFiles += childFileCount;
			} else {
				totalFiles++; // 如果是文件，计数加1
			}
		}
	...
	public override async generate(container?: HTMLElement): Promise<HTMLElement> {
		const wrapper = await super.generate(container);
...
export class FileTreeItem extends TreeItem {
	...
	protected async insertSelf(container: HTMLElement): Promise<HTMLElement> {
		...
		if (this.isFolder && this.fileCount > 0) {
			const countEl = self.createDiv({ cls: "nav-folder-count" });
			countEl.textContent = `${this.fileCount}`;
		}
